...
#ifdef BLOCKED
#   ifndef BLOCK_Z
#      define BLOCK_Z PRESET
#   endif
#   ifndef BLOCK_Y
#      define BLOCK_Y 4
#   endif
#   ifndef BLOCK_X
#      define BLOCK_X PRESET / 2
#   endif
#endif
                ...
                for (size_t bx = 0; bx < DIMX; bx += BLOCK_X) {
                    for (size_t z = bz; z < std::min(z + BLOCK_Z, DIMZ); ++z) {
                        for (size_t y = by; y < std::min(y + BLOCK_Y, DIMY); ++y) {
                            ...
                            #pragma omp simd
                            for (size_t x = bx; x < std::min(x + BLOCK_X, DIMX); ++x) {
...
auto main() -> int32_t {
    ...
    for (size_t i = 0; i < NB_ITERATIONS; ++i) {
        ...
        #ifdef BLOCKED
        printf(" %10.0lf %10.3lf %zu %zu %zu %d %d %d\n", elapsed, ns_point, DIMX, DIMY, DIMZ, BLOCK_Z, BLOCK_Y, BLOCK_X);
        #else
