        ...
        for (size_t z = 0; z < DIMZ; ++z) {
            for (size_t y = 0; y < DIMY; ++y) {
                ...
                for (size_t x = 0; x < DIMX; ++x) {
                    ...
                    float64_t acc = A[xyz] * B[xyz];
                    acc += A[(x - 1) + yz] * B[(x - 1) + yz] * exponents[0];
                    acc += A[(x - 2) + yz] * B[(x - 2) + yz] * exponents[1];
                    acc += A[(x - 3) + yz] * B[(x - 3) + yz] * exponents[2];
                    acc += A[(x - 4) + yz] * B[(x - 4) + yz] * exponents[3];
                    acc += A[(x - 5) + yz] * B[(x - 5) + yz] * exponents[4];
                    acc += A[(x - 6) + yz] * B[(x - 6) + yz] * exponents[5];
                    acc += A[(x - 7) + yz] * B[(x - 7) + yz] * exponents[6];
                    acc += A[(x - 8) + yz] * B[(x - 8) + yz] * exponents[7];
                    acc += A[(x + 1) + yz] * B[(x + 1) + yz] * exponents[0];
                    acc += A[(x + 2) + yz] * B[(x + 2) + yz] * exponents[1];
                    acc += A[(x + 3) + yz] * B[(x + 3) + yz] * exponents[2];
                    acc += A[(x + 4) + yz] * B[(x + 4) + yz] * exponents[3];
                    ...
                    acc += A[(x + 7) + yz] * B[(x + 7) + yz] * exponents[6];
                    acc += A[(x + 8) + yz] * B[(x + 8) + yz] * exponents[7];
                    acc += A[(y - 1) * MAXX + xz] * B[(y - 1) * MAXX + xz] * exponents[0];
                    acc += A[(y - 2) * MAXX + xz] * B[(y - 2) * MAXX + xz] * exponents[1];
                    acc += A[(y - 3) * MAXX + xz] * B[(y - 3) * MAXX + xz] * exponents[2];
                    acc += A[(y - 4) * MAXX + xz] * B[(y - 4) * MAXX + xz] * exponents[3];
                    acc += A[(y - 5) * MAXX + xz] * B[(y - 5) * MAXX + xz] * exponents[4];
                    acc += A[(y - 6) * MAXX + xz] * B[(y - 6) * MAXX + xz] * exponents[5];
                    acc += A[(y - 7) * MAXX + xz] * B[(y - 7) * MAXX + xz] * exponents[6];
                    acc += A[(y - 8) * MAXX + xz] * B[(y - 8) * MAXX + xz] * exponents[7];
                    acc += A[(y + 1) * MAXX + xz] * B[(y + 1) * MAXX + xz] * exponents[0];
                    acc += A[(y + 2) * MAXX + xz] * B[(y + 2) * MAXX + xz] * exponents[1];
                    acc += A[(y + 3) * MAXX + xz] * B[(y + 3) * MAXX + xz] * exponents[2];
                    acc += A[(y + 4) * MAXX + xz] * B[(y + 4) * MAXX + xz] * exponents[3];
                    ...
                    acc += A[(y + 7) * MAXX + xz] * B[(y + 7) * MAXX + xz] * exponents[6];
                    acc += A[(y + 8) * MAXX + xz] * B[(y + 8) * MAXX + xz] * exponents[7];
                    acc += A[(z - 1) * XY_PLANE + xy] * B[(z - 1) * XY_PLANE + xy] * exponents[0];
                    acc += A[(z - 2) * XY_PLANE + xy] * B[(z - 2) * XY_PLANE + xy] * exponents[1];
                    acc += A[(z - 3) * XY_PLANE + xy] * B[(z - 3) * XY_PLANE + xy] * exponents[2];
                    acc += A[(z - 4) * XY_PLANE + xy] * B[(z - 4) * XY_PLANE + xy] * exponents[3];
                    acc += A[(z - 5) * XY_PLANE + xy] * B[(z - 5) * XY_PLANE + xy] * exponents[4];
                    acc += A[(z - 6) * XY_PLANE + xy] * B[(z - 6) * XY_PLANE + xy] * exponents[5];
                    acc += A[(z - 7) * XY_PLANE + xy] * B[(z - 7) * XY_PLANE + xy] * exponents[6];
                    acc += A[(z - 8) * XY_PLANE + xy] * B[(z - 8) * XY_PLANE + xy] * exponents[7];
                    acc += A[(z + 1) * XY_PLANE + xy] * B[(z + 1) * XY_PLANE + xy] * exponents[0];
                    acc += A[(z + 2) * XY_PLANE + xy] * B[(z + 2) * XY_PLANE + xy] * exponents[1];
                    acc += A[(z + 3) * XY_PLANE + xy] * B[(z + 3) * XY_PLANE + xy] * exponents[2];
                    acc += A[(z + 4) * XY_PLANE + xy] * B[(z + 4) * XY_PLANE + xy] * exponents[3];
